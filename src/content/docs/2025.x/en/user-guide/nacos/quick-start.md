...
1. Starting from 2025.1.x, the Nacos module has officially dropped support for Spring Cloud Bootstrap. Please do not use this approach to configure or connect to Nacos.
2. Starting from 2025.0.x, if you are using the `public` namespace or a namespace with an empty string id in Nacos, please upgrade Nacos Server to version 3.x.
3. Starting from 2025.0.x, the Nacos module disables HealthIndicator checks by default. To enable it, explicitly set `spring.cloud.nacos.config.health-indicator.enabled=true` or `spring.cloud.nacos.discovery.health-indicator.enabled=true`.
...
2. The application/src/main/resources/application. The yaml configuration file configuration Nacos Config address and introduce the service configuration:
   ...
   ```
   * Since version 2023.0.1.3, `spring.cloud.nacos.config.shared-configs`, `extension-configs` and the default `application.name` configuration loading have been deprecated. Please use `spring.config.import` to import Nacos configuration, see issue [#3998](https://github.com/alibaba/spring-cloud-alibaba/issues/3998)
   * Starting from 2025.1.x, `bootstrap.yml` and `bootstrap.properties` are no longer supported. Please use `application.yml` or `application.properties` instead, see issue [#4166](https://github.com/alibaba/spring-cloud-alibaba/issues/4166)
