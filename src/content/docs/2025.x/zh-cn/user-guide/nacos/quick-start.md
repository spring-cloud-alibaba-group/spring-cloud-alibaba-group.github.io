...
1. 2025.1.x 版本开始，Nacos 模块正式废弃 Spring Cloud Bootstrap 的支持，请勿使用该方式配置和接入 Nacos。
2. 2025.0.x 版本开始，如果使用了 `public` 或者空字符串 id 的 Nacos 命名空间的情况下，请升级 Nacos Server 到 3.x 版本。
3. 2025.0.x 版本开始，Nacos 模块默认关闭 HealthIndicator 检查。如需开启，请显式配置 `spring.cloud.nacos.config.health-indicator.enabled=true` 或 `spring.cloud.nacos.discovery.health-indicator.enabled=true`。
...
2. 在应用的 /src/main/resources/application.yaml 配置文件中配置 Nacos Config 地址并引入服务配置：
   ...
   * 2025.1.x 开始，不再支持 `bootstrap.yml` 以及 `bootstrap.properties` 用法，请使用 `application.yml` 或者 `application.properties`，见 issue [#4166](https://github.com/alibaba/spring-cloud-alibaba/issues/4166)
