...
本章节展示 Spring Cloud Stream RocketMQ Binder 的实现和 RocketMQ 相关配置的高级特性。

## Spring Cloud Stream RocketMQ Binder 实现
...
Spring Cloud Stream Binder 去除了对 [RocketMQ-Spring](https://github.com/apache/rocketmq-spring) 框架的依赖 。 Spring Cloud Stream Binder 核心类 **RocketMQMessageChannelBinder** 实现了 Spring Cloud Stream 规范，内部会构建 RocketMQInboundChannelAdapter 和 RocketMQProducerMessageHandler。
...
### RocketMQ 绑定器配置
 ...
 关于 `spring-cloud-starter-stream-rocketmq-binder` 的 starter 配置项如下所示：
...
### RocketMQ 消息消费者配置
 关于 `spring-cloud-starter-stream-rocketmq-binder-consumer` 的 starter 配置项如下所示：

配置项|key|默认值|说明  
----|----|-----|-----  
|是否启用 Consumer|spring.cloud.starter.stream.rocketmq.binder.  consumer.enable| true| |
|Consumer 基于 TAGS 订阅|spring.cloud.starter.stream.rocketmq.binder.  consumer.subscription|empty|多个 tag 以 &#124&#124 分割。更多见 `RocketMQConsumerProperties.subscription`|
|Consumer 消费模式|spring.cloud.starter.stream.rocketmq.binder.  consumer.messageModel|CLUSTERING|如果想让每一个的订阅者都能接收到消息，可以使用广播模式。更多见 [oMessageModel](https://github.com/apache/rocketmq/blob/4.9.x/common/src/main/java/org/apache/rocketmq/common/protocol/heartbeat/MessageModel.java)|
...
关于 `spring-cloud-starter-stream-rocketmq-binder-consumer-push` 的 starter 配置项如下所示：

配置项|key|默认值|说明  
----|----|-----|-----  
是否同步消费消息模式|spring.cloud.starter.stream.rocketmq.binder.  consumer.push.pushorderly |false|
同步消费消息模式下消费失败重试策略|spring.cloud.starter.stream.rocketmq.binder.  consumer.push.delayLevelWhenNextConsume|0| -1，不重复，直接放入死信队列。0，broker 控制重试策略。0，client 控制重试策略
同步消费消息模式下消费失败后再次消费的时间间隔|spring.cloud.starter.stream.rocketmq.binder.  consumer.push.suspendCurrentQueueTimeMillis|1000|其他更多参数见 [RocketMQConsumerProperties.Push](https://github.com/alibaba/spring-cloud-alibaba/blob/2022.x/spring-cloud-alibaba-starters/spring-cloud-starter-stream-rocketmq/src/main/java/com/alibaba/cloud/stream/binder/rocketmq/properties/RocketMQConsumerProperties.java)

关于 `spring-cloud-starter-stream-rocketmq-binder-consumer-pull` 的 starter 配置项如下所示：
