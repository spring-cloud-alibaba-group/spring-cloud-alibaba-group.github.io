...
1.  需要在 `pom.xml` 文件中引入 group ID 为 `com.alibaba.cloud` 和 artifact ID 为 `spring-cloud-starter-stream-rocketmq` 的 starter：         
...
2. 更新 RocketMQ 中的主题（topic）为广播模式。  
  Linux/Unix/Mac 操作系统，执行命令 ：
    ```sh
...
3. 创建生产者：  
    ...
    ```
     **code**  
     使用 StreamBridge 和 ApplicationRunner 发送消息：  
